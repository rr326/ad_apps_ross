...
class BackupHa(adplus.Hass):
    ...
    def maybe_backup_now(self, _):
        ...
        if (self.get_now() - latest_backup_dt) > dt.timedelta(days=7) or self.test_mode:
            if self.test_mode:
                self.log(
                    f"DEBUG: running backup anyway. Actual age of oldest backup: {(self.get_now() - latest_backup_dt)} "
                )
            self.run_backup()
